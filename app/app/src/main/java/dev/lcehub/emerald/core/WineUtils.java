...
import dev.lcehub.emerald.container.Drive;
import dev.lcehub.emerald.win32.MSLogFont;
import dev.lcehub.emerald.win32.WinVersions;
...
public abstract class WineUtils {
    public static void createDosdevicesSymlinks(Container container, boolean addDriveCDRom) {
        ...
        String internalStorage = rootDir.getParentFile().getParentFile().getPath()+"/storage";
        ...
        for (Drive drive : container.drivesIterator()) {
            ...
            if (!linkTarget.isDirectory() && path.startsWith(internalStorage)) {
                linkTarget.mkdirs();
    ...
    public static void applySystemTweaks(Context context, WineInfo wineInfo) {
        ...
        try (WineRegistryEditor registryEditor = new WineRegistryEditor(userRegFile)) {
            ...
            registryEditor.removeKey("Software\\DiamondRuntime\\WFM\\ContextMenu\\7-Zip");
            registryEditor.setStringValue("Software\\DiamondRuntime\\WFM\\ContextMenu\\7-Zip", "Open Archive", "Z:\\opt\\apps\\7-Zip\\7zFM.exe \"%FILE%\"");
            registryEditor.setStringValue("Software\\DiamondRuntime\\WFM\\ContextMenu\\7-Zip", "Extract Here", "Z:\\opt\\apps\\7-Zip\\7zG.exe x \"%FILE%\" -r -o\"%DIR%\" -y");
            registryEditor.setStringValue("Software\\DiamondRuntime\\WFM\\ContextMenu\\7-Zip", "Extract to Folder", "Z:\\opt\\apps\\7-Zip\\7zG.exe x \"%FILE%\" -r -o\"%DIR%\\%BASENAME%\" -y");
            registryEditor.setStringValue("Software\\Wine\\AddonsURL", null, "https://raw.githubusercontent.com/LCE-Hub/DiamondRuntime/main/wine_addons/");
