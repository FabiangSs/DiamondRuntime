...
import dev.lcehub.emerald.xserver.XLock;
        ...
        Pointer.OnPointerMotionListener
{
    ...
    private volatile ArrayList<RenderableWindow> renderableWindows =
        new ArrayList<>();
    ...
    @FastNative
    private native void nativeUpdateWindowContent(
        long handle,
        long id,
    ...
    @FastNative
    private native void nativeUpdateWindowContentAHB(
        long handle,
        long id,
        long ahbPtr,
        short width,
    ...
    @FastNative
    private native void nativeSetTransformAndScissor(
        long handle,
        float ox,
        float oy,
        float sx,
    ...
    @FastNative
    private native void nativeUpdateCursorImage(
        long handle,
        java.nio.ByteBuffer pixels,
        short width,
        short height,
    ...
    @FastNative
    private native void nativeSetRenderList(
        long handle,
        long[] ids,
        int[] xs,
        int[] ys,
        ...
        if (fullscreen) {
            ...
            if (zoom != 1.0f) {
                ox =
                    ...
                    ptrX * viewTransformation.sceneScaleX * (1f - zoom);
                oy =
                    viewTransformation.sceneOffsetY +
                    ptrY * viewTransformation.sceneScaleY * (1f - zoom);
    ...
    private void collectWindows(
        ArrayList<RenderableWindow> list,
        Window window,
        int x,
        int y
    ) {
        ...
        if (window != xServer.windowManager.rootWindow) {
            ...
            if (viewable) list.add(
                new RenderableWindow(window.getContent(), x, y)
            );
        }
    ...
    private void pushRenderList(ArrayList<RenderableWindow> list) {
        ...
        final int screenW = xServer.screenInfo.width,
            screenH = xServer.screenInfo.height;
        int start = 0;
    ...
    public void onUpdateWindowContentDirect(
        Window window,
        Drawable pixmap,
        short xOff,
        short yOff
        ...
        synchronized (lock) {
            ...
            final int rx = window.getRootX() + xOff,
                ry = window.getRootY() + yOff;
            synchronized (pixmap.renderLock) {
                if (pixmap.getTexture() instanceof GPUImage) {
                    ...
                    if (ahbPtr != 0) {
                        nativeUpdateWindowContentAHB(
                            nativeHandle,
                            did(window.getContent()),
                            ahbPtr,
                            pixmap.width,
                            pixmap.height,
                            rx,
                    ...
                    if (vd != null) {
                        short s =
                            g.getStride() > 0 ? g.getStride() : pixmap.width;
                        nativeUpdateWindowContent(
                            nativeHandle,
                            did(window.getContent()),
                            vd,
                            pixmap.width,
        ...
        synchronized (lock) {
            ...
            synchronized (drawable.renderLock) {
                if (drawable.getTexture() instanceof GPUImage) {
                    ...
                    if (vd != null) {
                        short s =
                            g.getStride() > 0 ? g.getStride() : drawable.width;
                        nativeUpdateWindowContent(
                            nativeHandle,
                            did(drawable),
                            vd,
    ...
    public void onPointerMove(short x, short y) {
        synchronized (lock) {
            ...
            if (
                screenOffsetYRelativeToCursor || magnifierZoom != 1.0f
            ) updateTransform();
        }
    }
    ...
    public void setVerboseLog(boolean v) {
        synchronized (lock) {
            if (nativeHandle != 0) nativeSetVerboseLog(nativeHandle, v);
        }
    }
    ...
    public void setStretchMode(int mode) {
        ...
        synchronized (lock) {
            if (nativeHandle != 0) nativeSetStretchMode(nativeHandle, mode);
        }
    }
    ...
    public void setPostFXMode(int mode) {
        ...
        synchronized (lock) {
            if (nativeHandle != 0) nativeSetPostFXMode(nativeHandle, mode);
        }
    }
    ...
    public void setSharpness(float s) {
        pendingSharpness = s;
        synchronized (lock) {
            if (nativeHandle != 0) nativeSetSharpness(nativeHandle, s);
        }
    }
    ...
    public int[] getSupportedPresentModes() {
        synchronized (lock) {
            if (nativeHandle != 0) return nativeGetSupportedPresentModes(
                nativeHandle
            );
        }
        return new int[0];
    ...
    private static class RenderableWindow {
        ...
        public RenderableWindow(Drawable c, int x, int y) {
            content = c;
            rootX = x;
            rootY = y;
        }
